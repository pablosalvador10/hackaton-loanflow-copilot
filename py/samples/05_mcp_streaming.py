"""Sample 05: MCP Streaming — stream responses while using MCP tools.

Combines ``run_agent_stream()`` with MCP tools so you get incremental
text deltas in real time while the agent calls remote MCP tools in the
background.  MCP tool calls are auto-approved.
...
def main() -> None:
    ...
    # ── 2. Create agent and stream response ─────────────────────
